...
TraceFirstCwnd(std::ofstream* ofStream, uint32_t oldCwnd, uint32_t newCwnd)
{
        ...
        *ofStream << Simulator::Now().GetSeconds() << " " << static_cast<double>(newCwnd)/940
                  << std::endl;
...
NS_LOG_UNCOND ("TCP ADU size is: " << tcp_adu_size);
  ...
  //Config::SetDefault("ns3::TcpSocket::SndBufSize", UintegerValue(262144));//buffer size for 1st flow
...
//Config::SetDefault ("ns3::TcpSocket::SndBufSize", UintegerValue (1 << 21));
  ...
  Config::SetDefault("ns3::TcpSocket::InitialCwnd", UintegerValue(50));
  Config::SetDefault("ns3::TcpSocketBase::WindowScaling", BooleanValue(true));
  Config::SetDefault ("ns3::TcpSocketBase::Sack", BooleanValue (true));
  Config::SetDefault ("ns3::TcpSocket::DelAckCount", UintegerValue (0));
  Config::SetDefault ("ns3::BulkSendApplication::MaxBytes", UintegerValue (10000000));
  ...
  tchPfifo.SetRootQueueDisc ("ns3::PfifoFastQueueDisc", "MaxSize", StringValue ("1p"));
...
//NODE 1
   ...
   Address firstSinkAddress1S(InetSocketAddress(Ipv4Address::GetAny(), port));
     ...
     BulkSendHelper source2A ("ns3::TcpSocketFactory",Address());
    // Set the amount of data to send in bytes.  Zero is unlimited.
     //source2A.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
     InetSocketAddress firstDestAddress(id4.GetAddress(0), port);
     source2A.SetAttribute("Remote", AddressValue(firstDestAddress));
     ApplicationContainer sourceApps = source2A.Install(c.Get (1));
     sourceApps.Start (Seconds (11.0));
     ...
     Address firstSinkAddress2S(InetSocketAddress(Ipv4Address::GetAny(), port));
     ApplicationContainer firstSinkApp2S;
     PacketSinkHelper firstSinkHelper2S("ns3::TcpSocketFactory", firstSinkAddress2S);
     firstSinkApp2S = firstSinkHelper2S.Install(c.Get(4));
     firstSinkApp2S.Start (Seconds (10.0));
     firstSinkApp2S.Stop (Seconds (100));
    ...
    // Set the amount of data to send in bytes.  Zero is unlimited.
     ...
     //source1A.SetAttribute ("MaxBytes", UintegerValue (20000000));
     InetSocketAddress firstDestAddress1A(id3.GetAddress(0), port);
    ...
    //NODE 3
     ...
     firstSinkApp3S.Start (Seconds (20.0));
    ...
    // Set the amount of data to send in bytes.  Zero is unlimited.
     ...
     source3A.SetAttribute("Remote", AddressValue(firstDestAddress3A));
     ApplicationContainer sourceApps3A = source3A.Install(c.Get (2));
     sourceApps3A.Start (Seconds (21.0));
...
Ptr < Ipv4FlowClassifier > classifier = DynamicCast < Ipv4FlowClassifier >(flowmon.GetClassifier());
	...
	for (std::map < FlowId, FlowMonitor::FlowStats > ::const_iterator iter = stats.begin(); iter != stats.end(); ++iter) {
		...
		Time firstTxTime = iter->second.timeFirstTxPacket;
		Time lastTxTime = iter->second.timeLastTxPacket;
		Time delay1 = lastTxTime - firstTxTime;

		NS_LOG_UNCOND("Time First Packet Transmitted: " << firstTxTime.GetMilliSeconds() << " ms\n"
              << "Time Last Packet Transmitted: " << lastTxTime.GetMilliSeconds() << " ms\n"
              << "Delay Between First and Last Packet: " << delay1.GetMilliSeconds() << " ms");
  	 ...
  	 //std::cout << "  Tx Bytes:   " << iter->second.txBytes << "\n";
        ...
        // NS_LOG_UNCOND("Delay: " << delay << " ms");
         std::cout << "Drop Ratio: " << DropRatio*100.0/ rxPacket << "%" << "\n"; 
